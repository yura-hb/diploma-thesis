  ...
  public static void main(String[] args) {
    ...
    for (i=0; i<datasets.size(); i++) {
      for (j=0; j<algoritmos.size(); j++) {
	    ...
	    System.out.println("\\documentclass[a3paper,10pt]{article}\n" +
	    		"\\usepackage{graphicx}\n" +
	    		"\\usepackage{lscape}\n" +
					"\\usepackage[a3paper, total={10in, 8in}]{geometry}" +
	    		"\\title{Results}\n" +
	    		"\\author{}\n" +
	    		"\\date{\\today}\n" +
	    		"\\begin{document}\n" +
            ...
            "Algorithm&Ranking\\\\\n\\hline");
            for (i=0; i<algoritmos.size();i++) {
							String out = String.format("%.2f", Rj[i]);

              System.out.println((String)algoritmos.elementAt(i)+"&"+out+"\\\\");
            }
	    ...
	    System.out.println("\\end{lscape}\\end{document}");
	}
