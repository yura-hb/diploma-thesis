...
class GlobalMDPI(MachineReward):
    """
    ...
    class Configuration:
        ...
        @staticmethod
        def from_cli(parameters: Dict) -> 'GlobalMDPI.Configuration':
            return GlobalMDPI.Configuration(span=parameters.get('span', 256))
    ...
    def reward_after_completion(self, contexts: List[Context]):
        ...
        if contexts[0].job.is_tardy_upon_completion:
            ...
            if tardy_rate < 0.05:
                reward += tardy_rate * job.step_idx.shape[0]
            else:
                reward -= 5
        else:
            reward += job.step_idx.shape[0]
    ...
    def from_cli(parameters) -> MachineReward:
        return GlobalMDPI(GlobalMDPI.Configuration.from_cli(parameters))
